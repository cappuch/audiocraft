...
def submit():
    data = request.json
    text = data['text']
    melody = data.get('melody', None)  # Optional melody input
    # ... (other data extraction)
    
    # Process the text and melody inputs
    outputs = predict_full(model, text, melody, duration, topk, topp, temperature, cfg_coef)
    
    # Convert the output .wav file to base64 encoded string
    with open(outputs[0], 'rb') as wav_file:
        wav_data = wav_file.read()
        base64_wav = base64.b64encode(wav_data).decode('utf-8')
    
    return jsonify({'base64_wav': base64_wav})
